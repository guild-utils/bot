...
export default class extends Command{
    ...
    async run(message: KlasaMessage,[raw]:[string|KlasaMessage]): Promise<KlasaMessage | KlasaMessage[] | null>{
        let target:Message;
        ...
        return Promise.all(target.reactions.cache.array().map(async e=>{
            await e.users.fetch();
            return await message.sendMessage(
`
${e.emoji}\n
${e.users.cache.array().map(e=>message.guild!.members.resolve(e)!.displayName).join("\n")}
`
            );
        }));
    }
