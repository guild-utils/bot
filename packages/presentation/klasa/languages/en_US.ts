...
module.exports = class extends Language {
  ...
  private KLASA_MESSAGES = {
    ...
    COMMAND_LOAD_ERROR: (type, name, error) =>
      `❌ Failed to load ${type}: ${name}. Reason:${util.codeBlock(
        "js",
        error
      )}`,
    ...
    COMMAND_CONF_GUARDED: (name) =>
      `${util.toTitleCase(name)} may not be disabled.`,
  ...
  private PWRD_EVENTS_MESSAGES: PWRD_LANG_TYPE = {
    GOOGLE_SPREAD_SHEET_UPDATE_SUCCESS: "Updated.",
    INVALID_GOOGLE_SPREAD_SHEET_FORMAT:
      "Please specify the ID or URL of the google spread sheet.",
    NOTIFY_TO_NOTIFICATION_CHANNEL: (event, t, now) => {
      const diff = moment.duration(t.diff(now));
      return [
        `**${
          event.name
        }** starts in **${diff.minutes()}** minutes **${diff.seconds()}** seconds`,
        ...[...event.header]
          .filter((e) => e !== "name")
    ...
    COMMAND_LIST_DESCRIPTION:
      "Display a list of events in the collection with the given name, or a list of collections.",
    ...
    COMMAND_START_FAILED_WITH_BOT_NOT_JOINABLE:
      "Bot cannot connect to voice channel.",
    ...
    COMMAND_ADD_WORD_DESCRIPTION: "Add a word to the dictionary.",
