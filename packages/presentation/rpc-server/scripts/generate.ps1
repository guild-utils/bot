...
protoc --js_out=import_style=commonjs,binary:"$ABS_DIST_DIR" --ts_out=import_style=commonjs,binary:"$ABS_DIST_DIR" --grpc_out="$ABS_DIST_DIR" --plugin=protoc-gen-grpc="$ABS_PLUGIN_GRPC" --plugin=protoc-gen-ts="$ABS_PLUGIN_TS" --proto_path=./protos/ -I "$ABS_DIST_DIR" ./protos/*.proto