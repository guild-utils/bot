...
export default class extends CommandEx {
  ...
  async add(
    ...
    const append = () => {
      if (typeof arg1 === "number") {
        ...
        return this.dictionary.appendBefore(msg.guild!.id, arg2, arg1 - 1);
      } else {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        return this.dictionary.appendBefore(msg.guild!.id, arg1);
      }
    };
  ...
  async remove(
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const r = await this.dictionary.removeBefore(msg.guild!.id, arg1 - 1);
    if (r) {
  ...
  async update(
    ...
    arg2 = arg2 ?? "";
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const r = await this.dictionary.updateBefore(msg.guild!.id, arg1 - 1, arg2);
    if (r === undefined) {
      return msg.sendLocale(LANG_KEYS.INVALID_INDEX_RANGE);
    } else {
  ...
  async list(
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const p = await this.dictionary.getBefore(message.guild!.id);
    this.gui.emit("Init", {
      message,
      context: {
