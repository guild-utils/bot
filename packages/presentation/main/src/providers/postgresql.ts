...
export default class extends SQLProvider {
  ...
  countRows(table) {
    return this.runOne(
      `SELECT COUNT(*) FROM ${sanitizeKeyName(table)};`
  ...
  getAll(table, entries = []) {
    if (entries.length) {
      return this.runAll(
        ...
        results.map((output) => this.parseEntry(table, output))
      );
    }
    return this.runAll(
  ...
  getKeys(table) {
    return this.runAll(
      `SELECT id FROM ${sanitizeKeyName(table)};`
  ...
  has(table, id) {
    return this.runOne(
      `SELECT id FROM ${sanitizeKeyName(table)} WHERE id = $1 LIMIT 1;`,
      [id]
  ...
  getRandom(table) {
    return this.runOne(
      `SELECT * FROM ${sanitizeKeyName(table)} ORDER BY RANDOM() LIMIT 1;`
    );
  }
  ...
  getSorted(table, key, order = "DESC", limitMin, limitMax) {
    return this.runAll(
      `SELECT * FROM ${sanitizeKeyName(table)} ORDER BY ${sanitizeKeyName(
        key
      )} ${order} ${parseRange(limitMin, limitMax)};`
    );
  }
  ...
  incrementValue(table, id, key, amount = 1) {
    return this.run(
      `UPDATE ${sanitizeKeyName(table)} SET $2 = $2 + $3 WHERE id = $1;`,
      [id, key, amount]
    );
  }
  ...
  decrementValue(table, id, key, amount = 1) {
    return this.run(
      `UPDATE ${sanitizeKeyName(
        table
      )} SET $2 = GREATEST(0, $2 - $3) WHERE id = $1;`,
      [id, key, amount]
    );
  }

  delete(table, id) {
    return this.run(`DELETE FROM ${sanitizeKeyName(table)} WHERE id = $1;`, [
      id,
    ]);
  }
  ...
  updateColumn(table, piece) {
    ...
    return this.run(
      `ALTER TABLE ${sanitizeKeyName(
        ...
        piece.default
          ? `, ALTER COLUMN ${column} SET NOT NULL, ALTER COLUMN ${column} SET DEFAULT ${this.qb.parseValue(
              piece.default,
              piece
            )}`
          : ""
  ...
  runAll(...sql) {
    return this.run(...sql).then((result) => result.rows);
  }

  runOne(...sql) {
    return this.run(...sql).then((result) => result.rows[0]);
  }
}
...
function parseRange(min, max) {
  ...
  if (min < 0) {
    throw new RangeError(
      `[PARSE_RANGE] 'min' parameter expects to be equal or greater than zero, got ${min}`
    );
  }
  ...
  if (typeof max !== "undefined") {
    ...
    if (max <= min) {
      throw new RangeError(
        `[PARSE_RANGE] 'max' parameter expects ${max} to be greater than ${min}. Got: ${max} <= ${min}`
      );
    }
  }
