...
export default class MemberGateway extends Gateway {
	...
	get(id: string, create?: boolean, makingGuild?: Guild): Settings;
	get(id: string|string[], create: boolean = false, makingGuild?: Guild): Settings {
	...
	async sync(input: string[]|string = [...[...this.client.guilds.cache.values()].flatMap(guild => [...guild.members.cache.values()].map(member => guild.id + '.' + member.user.id))]): Promise<Gateway|Settings> {
		if (Array.isArray(input)) {
			...
			for (const guild of this.client.guilds.cache.values()) {
				for ( const entry of guild.members.cache.values()) {
					if ((entry.settings as any)._existsInDB === null) (entry.settings as any)._existsInDB = false;
				}
			}
			return this;
		}
