...
export default class extends KlasaClient {
	...
	static [KlasaClient.plugin]() {
		...
		const memberSchema = 'schema' in members ? members.schema : (this.constructor as any).defaultMemberSchema;
		const provider = members.provider || self.options.providers.default;
		if (!provider) {
			throw new Error('klasa-member-gateway initialized error');

		}
		self.gateways.members = new MemberGateway(self.gateways, 'members', memberSchema, provider);
		self.gateways.keys.add('members');
		self.gateways._queue.push(self.gateways.members.init.bind(self.gateways.members));
	}
