...
export class GameEventNotificationRepositoryKlasa
  implements GameEventNotificationRepository {
  ...
  async register(guildId: string, event: GameEvent[]): Promise<void> {
    ...
    const taskId: string | undefined = settings.get(this.nextTaskId);
    const bt = taskId ? sc.get(taskId) : null;
    const now = moment.utc();
    const newt = event
      .map((e): [GameEvent, moment.Moment | undefined] => [
        e,
        nextNoticeTime(e, now),
      ])
      .filter(
        (
          e: [GameEvent, moment.Moment | undefined]
        ): e is [GameEvent, moment.Moment] =>
          e[1] !== undefined && e[0].lastNotice.isBefore(e[1])
      )
      .sort(([, at], [, bt]) => at.diff(bt))[0];
    if (newt === undefined) {
      return;
    }
    if (!bt) {
